...
describe('validatePrompt', () => {
  ...
  it('Errors on high number', () => {
    expect(() => validatePrompt(20, played)).toThrow('Number must be in range 1 - 9');
  });

  it('Errors on low number', () => {
    expect(() => validatePrompt(-1, played)).toThrow('Number must be in range 1 - 9');
  });

  it('Errors on played square', () => {
    played['X'].add(1);
    expect(() => validatePrompt(1, played)).toThrow('Square 1 has already been played. Select another square');
  });
});
